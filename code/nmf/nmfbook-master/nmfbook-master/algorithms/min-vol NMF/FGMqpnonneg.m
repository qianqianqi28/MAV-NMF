...
% Fast gradient method to solve quadratic programs with nonnegativity constraints.  
...
% This assumes A is full rank, which is always the case in the context of 
% min-vol NMF when lambda > 0. 
...
while i <= maxiter && eps >= delta*eps0
    ...
    % Projection 
    if proj == 1
        W = max(W,0); % Projection onto the nonnegative orthant
    elseif proj == 2
        W = SimplexColProj( W ); % W is projected to become column stochastic
    elseif proj == 3
        W = SimplexColProj( W' ); % W is projected to become column stochastic
        W = W';
    end
