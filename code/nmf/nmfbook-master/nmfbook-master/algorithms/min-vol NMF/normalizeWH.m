...
elseif sumtoone == 5 % Normalize so that W e = e, 
                     % % entries in rows of W sum to 1  
    wtranspose = W';
    wntranspose = SimplexColProj( wtranspose );

    if norm(wntranspose - wtranspose) > 1e-3*norm(wntranspose); 
       W = wntranspose'; 
       % reoptimize H, because this normalization is NOT w.l.o.g. 
       options.inneriter = 100; 
       options.H = H; 
       H = nnls_FPGM(X,W,options); 
    end
    W = wntranspose';   
end