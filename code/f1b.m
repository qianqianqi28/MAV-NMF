...
plot(W_min_triangle(:,1), W_min_triangle(:,2), 'b--', 'LineWidth', 5);

data_points_x = plot(ternary_to_xy_X(1,:), ternary_to_xy_X(2,:), 'bo', 'MarkerSize', 8, 'LineWidth', 5);

text(Wt_triangle(1,1)-0.2, Wt_triangle(1,2)-0.08, '$\textbf{M}_\textbf{1}\textbf{(:, 1)}$', 'Interpreter','latex','FontSize',30);
text(Wt_triangle(2,1)-0.08,      Wt_triangle(2,2)-0.03, '$\textbf{M}_\textbf{1}\textbf{(:, 2)}$', 'Interpreter','latex','FontSize',30);
text(Wt_triangle(3,1),  Wt_triangle(3,2)+0.015, '$\textbf{M}_\textbf{1}\textbf{(:, 3)}$', 'Interpreter','latex','FontSize',30);

text(W_min_triangle(1,1)-0.2, W_min_triangle(1,2)+0.05, '$\textbf{M}_\textbf{2}\textbf{(:, 1)}$', 'Interpreter','latex','FontSize',30);
text(W_min_triangle(2,1),      W_min_triangle(2,2)+0.01, '$\textbf{M}_\textbf{2}\textbf{(:, 3)}$', 'Interpreter','latex','FontSize',30);
text(W_min_triangle(3,1)-0.02, W_min_triangle(3,2)+0.01, '$\textbf{M}_\textbf{2}\textbf{(:, 2)}$', 'Interpreter','latex','FontSize',30);

exportgraphics(gcf,'plots/f1b.pdf','ContentType','vector');
